...
function ganjilGenapAsianGames(date, data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (vehicle.type === 'Mobil') {
            let platNomor = vehicle.plat.split(' ');
            let NomorTerakhir = platNomor[platNomor.length - 2];
            let DigitTerakhir = parseInt(NomorTerakhir[NomorTerakhir.length - 1]);

            if ((date % 2 === 0 && DigitTerakhir  % 2 !== 0) || (date % 2 !== 0 && DigitTerakhir % 2 === 0)) {
                pelanggaran++
            }
        }
    }
